...
CLIENT =	${SSH} ${SUDO} ./sudpclient ${ONESHOT} -4v -a 30 -i 10 -r 3 -w 10 -n 300 -p 4000
SERVER =	${SUDO} ./sudpserver ${ONESHOT} -4sv -i 10 -n 1000 -p 4000 ${BIND}
...
stamp-pfctl:
	echo pass out proto udp to ${LOCAL_ADDR} port { ${PORT1} ${PORT2} } \
	    keep state '(udp.first 10, udp.single 5, udp.multiple 60)' \
	    divert-reply \
